...
def run_reactants(rxn, reactants):
    ...
    try:
        ...
        out=('Error: Applying transform failed.\n')
        return False, out
    ...
    if len(IsMatchChiral) != len(productList):
        out=('Error: Template maps does not match products.\n')
        return False, out
...
def unique_products(transformSMARTS, reactantSMILES):
    ...
    # Run Reaction
    ps,out = run_reactants(rxn, reactants)
...
# Simple sanity checks. Never trust a user! ;-)
    ...
    if len(reactant_smarts) != len(smiles):
        out='Error: transform cannot be applied: invalid number of reactant SMILES.<br>\n input smiles:' + str(smiles)
        out=out+"<br>lenReactantSmarts= " + str(len(reactant_smarts)) +" len smiles = " + str(len(smiles)) +smarts
        return False, out, {}
    ...
    try:
        ...
        out='Error: Invalid transform SMARTS.\n ' #+ smarts
        out=out + "\n<br> : " + str(type(smarts))
        return False, out, {}
    ...
    stuff = {'transform':smarts, 'reactants':smiles, 'products':uniqueProductsList} 
    return True, out, stuff
